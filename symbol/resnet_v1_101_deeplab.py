...
class resnet_v1_101_deeplab(Symbol):
    ...
    def get_resnet_conv(self, data):
        conv1 = mx.symbol.Convolution(name='conv1', data=data, num_filter=64, pad=(3, 3), kernel=(7, 7), stride=(1, 1),
                                      no_bias=True)
    ...
    def get_train_symbol(self, num_classes):
        ...
        heatmap_label = mx.sym.Variable("heatmaplabel")
        ...
        score = mx.symbol.Convolution(data=relu_fc6, kernel=(1, 1), pad=(0, 0), num_filter=numofparts + 2 * numoflinks, name="score",
                                      bias=score_bias, weight=score_weight, workspace=self.workspace)

        heatmap_score = mx.symbol.slice_axis(score, axis=1, begin=0, end=numofparts)
        heatmap_score = mx.symbol.sigmoid(heatmap_score)
        loss_heatmap = (heatmap_label * mx.symbol.log(heatmap_score + 1e-12) +
                        (1 - heatmap_label) * mx.symbol.log(1 - heatmap_score + 1e-12))/(46 * 46 * numofparts)
        loss_heatmap = loss_heatmap * -1.0
        # loss_heatmap = (heatmap_label - heatmap_score) ** 2 / (46 * 46 * numofparts)
        pafmap_score = mx.symbol.slice_axis(score, axis=1, begin=numofparts, end=numofparts + numoflinks * 2)

        loss_paf = (pafmap_score - partaffinityglabel) ** 2 / (46 * 46 * numoflinks * 2)

        return mx.symbol.Group([ mx.symbol.MakeLoss(loss_heatmap * heatweight), mx.symbol.MakeLoss(loss_paf * vecweight)])
...
def get_module(num_classes = 19*3,for_training = False,input_shape = (368,368),feature_shape = (46,46)):
    Sym = resnet_v1_101_deeplab()
    sym = Sym.get_symbol(num_classes = 14)
    mx.visualization.plot_network(sym, shape={"data": (10, 3, 368, 368)}).view()
    pass
